...
function showError(msg){
  ...
  el.innerHTML = `<div style="padding:14px; color:#e5e7eb; font-family:system-ui;">
    ...
    <div style="margin-top:10px; color:#9ca3af;">Tip: abre DevTools (F12) → Console/Network para ver el error exacto.</div>
  </div>`;
}
...
function buildPlot(){
  try{
    if(typeof Plotly === "undefined"){
      showError("Plotly no está disponible. Revisa si el CDN cargó (plotly-*.min.js).");
      return;
    }
    ...
    if(showClim && CLIM.length > 0 && ["Tmin_C","Tmax_C","Tavg_C","PRCP_mm"].includes(metric)){
      const filtered = CLIM.filter(r => {
        const d = toNum(r["DOY_365"]);
        return d !== null && d >= startDoy && d <= endDoy;
      });
      const x = filtered.map(r => toNum(r["DOY_365"])).filter(v => v !== null);
...
async function safeFetchText(url){
  ...
  if(!r.ok){
    throw new Error(`No se pudo cargar ${url}. HTTP ${r.status}`);
  }
  return await r.text();
}
...
async function init(){
  try{
    const [dataTxt, climTxt] = await Promise.all([
      ...
      safeFetchText(CLIM_URL).catch(err => {
        // Climatology is optional
        console.warn(err);
        return "";
      })
    ]);
    ...
    const btnNovDec = document.getElementById("btnNovDec");
    const btnJJA = document.getElementById("btnJJA");
    if(btnFull) btnFull.addEventListener("click", () => setIntervalMD(1,1,12,31));
    if(btnNovDec) btnNovDec.addEventListener("click", () => setIntervalMD(11,1,12,31));
    if(btnJJA) btnJJA.addEventListener("click", () => setIntervalMD(6,1,8,31));
